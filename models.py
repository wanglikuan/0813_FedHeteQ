...
class LeNet_Q(nn.Module):
    def __init__(self, dataset='mnist', model='cnn_Lenet', feature_dim=50*4*4, bottleneck_dim=256, num_labels=10, iswn=None):
        ...
        self.Linear_Q = nn.Linear(num_labels, num_labels, bias=False)
...
class DigitModel_Q(nn.Module):
    def __init__(self, class_num=10):
        super(DigitModel_Q, self).__init__()
        self.class_num = class_num
        ...
        self.fc3 = nn.Linear(512, class_num)
        self.fcq = nn.Linear(class_num, class_num)
...
class LeNet_CIFAR(nn.Module):
    def __init__(self, dataset='cifar', model='cnn_Lenet', feature_dim=50*4*4, bottleneck_dim=256, num_labels=10, iswn=None):
        ...
        # self.bn = nn.BatchNorm1d(bottleneck_dim, affine=True)
        # self.dropout = nn.Dropout(p=0.5)
        self.bottleneck = nn.Linear(16 * 8 * 8, 84)
        # self.bottleneck.apply(init_weights)
        self.fc = nn.Linear(84, num_labels)
        if iswn == "wn":
            self.fc = nn.utils.weight_norm(self.fc, name="weight")
...
class CNNTarget_bn(nn.Module):
    def __init__(self, in_channels=3, n_kernels=16, out_dim=10):
        super(CNNTarget_bn, self).__init__()
