...
def test_and_save(label_id, client_id, data, test_sample_num=100): ### label_id是要跑的data的label；client_id是受测试的client的id ###
    device = "cuda"
    id, train_data , test_data = read_user_data(label_id, data, dataset='Mnist-class1') #输出的 train 为指定label数据
...
########## 准备网络 ##########
    ...
    resnet_18.load_state_dict(torch.load('fedavg_net_client{}.pt'.format(client_id), map_location='cpu')) ### 加载指定模型参数 ###
