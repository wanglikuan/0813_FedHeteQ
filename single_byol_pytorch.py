...
####
        ...
        self.train_transform = torchvision.transforms.Compose(
            [
                ...
                torchvision.transforms.RandomResizedCrop(size=56),
...
####
    ...
    def forward(
        ...
        assert not (self.training and x_i.shape[0] == 1), 'you must have greater than 1 sample when training, due to the batchnorm in the projection layer'

        if return_embedding:
            return self.online_encoder(x_i, return_projection = return_projection)

        #image_one, image_two = self.augment1(x), self.augment2(x)
        #image_one, image_two = self.train_transform(x), self.train_transform(x)
        image_one, image_two = x_i, x_i
